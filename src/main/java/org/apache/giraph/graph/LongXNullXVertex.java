...
/**
 * Simple implementation of {@link Vertex} using an long as id, and having
 * vertex value and message type as generic arguments. Edges are immutable and
 * unweighted. This class aims to be as memory efficient as possible. This class
 * is based on {@link IntIntNullIntVertex} and {@link HashMapVertex}.
 */
public abstract class LongXNullXVertex<V extends Writable, M extends Writable>
    extends Vertex<LongWritable, V, NullWritable, M> {
  ...
  public void setNeighbors(Set<LongWritable> neighborSet) {
    neighbors = new long[(neighborSet != null) ? neighborSet.size() : 0];
    int n = 0;
    if (neighborSet != null) {
      for (LongWritable neighbor : neighborSet) {
  ...
  @Override
  public void setEdges(Map<LongWritable, NullWritable> edges) {
    setNeighbors(edges.keySet());
  }
